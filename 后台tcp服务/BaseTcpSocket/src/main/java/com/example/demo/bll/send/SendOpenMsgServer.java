...
@Log
@Component
public class SendOpenMsgServer extends Thread {
    ...
    public void run() {
        ...
        try(Jedis redis = RedisHelper.getJedis()) {
            ...
            String redis_open_pc_key = conf.getRedis_open_pc_key();
            String message = "201";
            String devId = null;
            //监听消息队列
            while (true) {
                try {
                    devId = redis.brpop(0,redis_open_pc_key).get(1);
                }
                catch (Exception e) {
                    log.info("开机信息发送程序 未获取redis的数据,异常信息: " + e.getMessage());
                    Thread.sleep(1000);
                    devId = null;
                }
                ...
                {
                    log.info("开机信息发送程序 报错,异常信息: " + e.getMessage());
                }
            }
        }
        catch (Exception e) {
            log.info("开机信息发送程序 异常关闭,异常信息: " + e.getMessage());
        }
    }
}